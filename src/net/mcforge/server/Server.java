/**
 * *****************************************************************************
 * Copyright (c) 2012 GamezGalaxy. All rights reserved. This program and the
...
=======
    ...
    public String flags;
    /**
     * The MoTD of the server (The message the player sees when first joining
     * the server)
     */
    public String MOTD;
    /**
     * The main level (The level the user first joins when the player joins the
     * server)
     */
    public String MainLevel;
    /**
    ...
    /**
     * The handler that handles level loading, level unloading and finding
     * loaded levels
     *
     * @return The {@link LevelHandler}
     */
    public final LevelHandler getLevelHandler() {
        return lm;
    }
    ...
    /**
     * The handler that handles events. Use the EventSystem to register event or
     * call events
     *
     * @return The {@link EventSystem}
     */
    public final EventSystem getEventSystem() {
        return es;
    }
    ...
    /**
     * Get the handler that handles the player commands. Use this to add/remove
     * commands or excute commands
     *
    ...
    /**
     * The SQL object where you can execute Queries
     *
     * @return The {@link ISQL} object
     */
    public final ISQL getSQL() {
        return sql;
    }
    ...
    /**
     * Get the object that controls the updating of plugins and other {@link Updatable}
     * objects.
     *
     * @return The {@link UpdateService} object
     */
    public final UpdateService getUpdateService() {
        return us;
    }
    ...
    /**
     * The constructor to make a new {@link Server} object
     *
     * @param Name The default name of the server. This will be changed if the
     * properties file has something different.
     * @param Port The default port of the server. This will be changed if the
     * properties file has something different.
     * @param MOTD The MoTD message for the server. This will be changed if the
     * properties file has something different.
     */
    public Server(String Name, int Port, String MOTD) {
        this.Port = Port;
    ...
    /**
     * Get the salt. <b>This method can only be called by heartbeaters and the
     * Connect Packet. If this method is called anywhere else, then a {@link IllegalAccessException}
     * is thrown</b>
     *
     * @return The server Salt
     * @throws IllegalAccessException This is thrown when an attempt to call
    ...
    /**
     * Load the server properties such as the server {@link Server#Name}. These
     * properties will always load from the {@link Server#configpath}
     */
    public void loadSystemProperties() {
        Name = getSystemProperties().getValue("Server-Name");
        altName = getSystemProperties().getValue("WOM-Alternate-Name");
    ...
    /**
     * Start the logger. The logger can be started before the server is started,
     * but this method is called in {@link Server#Start()}
     */
    public void startLogger() {
        Calendar cal = Calendar.getInstance();
        cal.clear(Calendar.HOUR);
     ...
     * Start the SQL service
     *
     * @param set The SQL interface to use
     */
    public void startSQL(ISQL set) {
        if (set == null) {
            try {
    ...
    /**
     * Search for a player based on the name given. A part of the name will be
     * given and will find the full name and player. If part of the name is
     * given, and more than 1 player is found, then it will return null.
     *
     * @param name The full/part name of the player
     * @return The player found. If more than 1 player is found, then it will
     * return null.
     */
    public Player findPlayer(String name) {
        Player toreturn = null;
    ...
    /**
     * Stop the server, this will kick all the players on the server and stop
     * all server services.
     *
     * @throws InterruptedException if any thread has interrupted the current
     * thread. The interrupted status of the current thread is cleared when this
     * exception is thrown.
     * @throws IOException If there is a problem saving the levels that are
     * loaded
     */
    public void Stop() throws InterruptedException, IOException {
        if (!Running) {
     ...
     * Add a task to be called every 10 milliseconds
     *
     * @param t The {@link Tick} object to call
     */
    public void Add(Tick t) {
        synchronized (ticks) {
            if (!ticks.contains(t)) {
     ...
     * Remove a task from the Tick list
     *
     * @param t The {@link Tick} object to remove
     */
    public void Remove(Tick t) {
        synchronized (ticks) {
            if (ticks.contains(t)) {
...
>>>>>>> 9bfcc1b493d438cb4e1786870be3099235a88c54
}